...
var artists = function(page, artist, details, callback) {
  db.all("SELECT Artist.ArtistId, Name, StarsNo " +
    ...
    "ORDER BY Name LIMIT " + artistsPerPage + " OFFSET ($page - 1) * " + artistsPerPage,
    {$page: page}, function(error, rows) {
      if (error) {
        ...
        for (var i = 0; i < rows.length; i++) {
          ...
          result += '<div id="' + rows[i].ArtistId + '"><span class="numbers">' + (page * artistsPerPage + i - (artistsPerPage-1)) + '.</span>' +
